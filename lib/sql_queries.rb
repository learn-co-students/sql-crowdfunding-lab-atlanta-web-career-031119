...
"SELECT users.name,users.age,SUM(pledges.amount) FROM users LEFT JOIN pledges ON pledges.user_id = users.id GROUP by users.name;"
end
...
"SELECT users.name, SUM(pledges.amount) FROM users LEFT JOIN pledges ON users.id = pledges.user_id group by users.name ORDER BY sum(pledges.amount),users.name "
end
...
"SELECT projects.category, sum(pledges.amount) FROM projects LEFT JOIN pledges ON pledges.project_id = projects.id where projects.category = 'books' Group by projects.category;"
end
